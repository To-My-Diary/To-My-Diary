...
//    public void createDetailGoal(DetailGoalDto detailGoalDto) {
...
//    }
    ...
    public List<CalendarGoalDto> getCalendarGoal(int year, int month, String userId) {
        ...
        // 색깔과 날짜 담기 - 메인 목표
        for (Goal goal : goals) {
            CalendarGoalDto dto = new CalendarGoalDto(Boolean.TRUE, goal.getColor(),
                    goal.getPlanDate().getDayOfMonth());
            days.add(dto);
        }
        // 색깔과 날짜 담기 - 목표 상세
        for (DetailGoal detailGoal : detailGoals) {
            CalendarGoalDto dto = new CalendarGoalDto(Boolean.FALSE, detailGoal.getColor(),
                    detailGoal.getPlanDate().getDayOfMonth());
            days.add(dto);
        }
        return days;
    }
